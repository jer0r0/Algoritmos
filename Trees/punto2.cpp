...
class Btree{
    ...
    public:
        ...
        void insert_inv(T k){
            insert_rec_inv(k, root, nullptr);
        }
        void insert_rec_inv(T k, Node<T>* &c, Node<T> *p){
            if(c == nullptr){
                c = new Node<T>;
                c->key = k;
                c->right = nullptr;
        ...
        void print_rec_2(Node<T> *n, int level, int h){
            if(n != nullptr){
                if(level <= h){
                    print_rec_2(n->left, level + 1,h);
                    std::cout << n->key << "(" << level << ")\t";
                    print_rec_2(n->right, level + 1,h);
                }
            }
        }
        void print_2(){
            int h = 0;
            cout<<"¿Que numero desea que sea h?"<<"\n";
            cin>>h;
            print_rec_2(root, 0,h);
            cout<<endl;
        }
        
        void h2_rec(Node<T>* n){
            int h = 0;
            cout<<"¿Que numero desea que sea h?"<<"\n";
            cin>>h;
            ...
            if(n != nullptr){
                if(altura(n->right)<=h){
                    print_rec_2(n->right, 0, h);
                }
                if(altura(n->left)<=h){
                    print_rec_2(n->left, 0, h);
                }
            }
        }
...
int main()
{
    ...
    mytree.insert(13);
    mytree.insert(16);
    cout<<"imprimiendo el arbol normal"<<endl;
    mytree.print_2();
